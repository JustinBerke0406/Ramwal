...
public class DeriverTileEntity extends TileEntity implements INamedContainerProvider, ITickableTileEntity {
    ...
    public DeriverTileEntity() {
        super(TileEntityInit.DERIVER.get());

        input = ZoneContents.createForTileEntity(INPUT_SLOTS_COUNT, this::canPlayerAccessInventory, this::markDirty);
        outputs = ZoneContents.createForTileEntity(OUTPUT_SLOTS_COUNT, this::canPlayerAccessInventory, this::markDirty);
    ...
    public Container createMenu(int p_createMenu_1_, PlayerInventory p_createMenu_2_, PlayerEntity p_createMenu_3_) {
        return DeriverContainer.createContainerServerSide(p_createMenu_1_, p_createMenu_2_, input, outputs, fuel);
    }
    ...
    public CompoundNBT write(CompoundNBT parentNBTTagCompound)
    {
        ...
        parentNBTTagCompound.put(INPUT_SLOTS_NBT, input.serializeNBT());
        parentNBTTagCompound.put(OUTPUT_SLOTS_NBT, outputs.serializeNBT());
    ...
    public void read(BlockState blockState, CompoundNBT nbtTagCompound)
    {
        ...
        input.deserializeNBT(inventoryNBT);

        inventoryNBT = nbtTagCompound.getCompound(OUTPUT_SLOTS_NBT);
        outputs.deserializeNBT(inventoryNBT);

        inventoryNBT = nbtTagCompound.getCompound(FUEL_SLOTS_NBT);
        fuel.deserializeNBT(inventoryNBT);

        if (input.getSizeInventory() != INPUT_SLOTS_COUNT
                || outputs.getSizeInventory() != OUTPUT_SLOTS_COUNT
    ...
    public void dropAllContents(World world, BlockPos blockPos) {
        InventoryHelper.dropInventoryItems(world, blockPos, input);
    ...
    public ITextComponent getDisplayName() {
        return new TranslationTextComponent("container.ramwal.deriver");
    }
}
